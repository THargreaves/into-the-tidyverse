...
2. Remember to specify the `NA` character `'n/a'`
...
3. Are there any there are flowers in the `iris` dataset with petals that are wider than they are long?
4. What is the carat of the highest priced diamond in the the `diamond` dateset? (A condition such as `price == max(price)` may be useful)
...
2. What was the interquartile range of the mean windspeed for each day?
3. How many days did the wind blow in a south-south-easterly direction?
4. Select all observations that occured after 2015-10-23 (use `date > 2015-10-23` as your comparison)
...
1. Now using `arrange()` find the car in the `mpg` dataset with the best city milage. What about the worst?
...
2. Use `arrange()` to put the observation with missing mean windspeed at the top. Hint: use `is.na()` remembering that `FALSE` comes before `TRUE` in ordering
...
4. Order the dataset by the 'complexity' of `max_gust_direction_cardinal` (number of letters in the direction name). `nchar()` may be helpful - see the first example in the corresponding help page
...
1. Select all columns from the weather dataset that with `mean_` as well as the date
...
1. Create a new column called `cty_km_l` which measures the city mileage in kilometers per litre (Hint: 1 mile ~ 1.6 km, 1 gallon ~ 3.8 litres)
...
7. Create a new colum in the `diamonds` dataset called `expensive` which is `TRUE` if and only if the price of the diamond is above the upper quartile of all prices

#### The Weather Dataset

1. Calculate the cummulative total rainfall for each day in the weather dataset. What about the cummulative maximum?
2. Overwrite the `mean_temp` column by converting to fahreheit (see presentation for conversion)
3. Look at the help document for `lag()` and `lead()`. Use one of these to find the daily difference in mean pressure
4a. **(HARD)** There are proper ways to handle date columns but these can get quite complicated. A simple trick is to use the `substr()` function. Read the help docs on this function and then try to use it to create a new variable containing the month in numeric form (though stored as a character)
4b. You can then convert these characters to integers by using another mutate with the `as.integer()` function
4c. To convert these to dates the last step is to use either the `month.abb` or `month.name` built-in object with a mutate. Something similar to this will do: `month_word_col = month.name[month_number_col]`, replacing `month_word_col`, `month_number_col` with desired names
5. **(VERY HARD)** What is the output of `as.integer(weather$date)`? Create a column in the dataset representing the numerical form of the date.
April 1st 2015 was a Wednesday, use some modular arithmatic to create a column containing the date of the week as a number - e.g. Thursday = 4. (Hint: `(date_numeric + 1) %% 7 + 1`)
...
6. For each cut and color in the diamonds dataset, what is the range (`diff(range(...))`) of the carats?

#### The Weather Dataset

1. What was the median total rainfall over the span of the dataset?
2. What is the mean windspeed for each Beaufort classification
...
1. Looking only at the month of April, what was the highest mean pressure for each day of the week? Order by pressure
...
3. For each year in the `mpg` dataset, rank the cars by city mileage so that the lowest milage is ranked first. Select the cars that are ranked number one for each year
