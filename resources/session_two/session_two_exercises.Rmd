...
Remember, before you can use `ggplot2`, you need to load the package.
...
2. Create a new vector `squares` which contains the square of every value in `x`. Remember most mathematical functions in R act on each element individually. You might want to use the `^` operator.
...
4. Use `ggplot()` to produce either a scatter or line plot of these two variables

#### R as a Calculator 2

1. Create a new sequence called `x` containing 1000 numbers spaced uniformly between -6 and 6. You can do this by adding the `length.out` parameter to `seq()` and setting it to `1000` (this parameter must be specified by name)
...
1. Which parameters from the following code can you call without specifying their name? 
...
1. Read the `people.csv` file from this session's `data` folder. Make sure you specify that `sex` and `race` are factors (categorical variables) and that `earn` and `age` or integers.
2. How does level of education affect salary? Make a scatter plot to find out. You should set `position = 'jitter'` in the `geom_point()` function to avoid overplotting and perhaps also set transparency to `0.5`.
3. Facet the above plot by `sex`, colouring the points too. Make sure you hide the legend with `show.legend = FALSE`
4. Label the plot using the `labs()` function
...
5. How is total rainfall affected by cloud coverage (`mean_cloud`)? Make a scatter plot to find out. Set `position = 'jitter'` to avoid overplotting
...
5. Make a scatter plot of `Worldwide Gross` against `Audience score %` coloured by `Genre`. When using a column name with a space you need to surround the name with backticks (\`)
...
5. Overlay the data points coloured by diet also. Add a jitter to prevent overfitting and reduce transparency so this doesn't distract from the trend lines
6. Label the plot
