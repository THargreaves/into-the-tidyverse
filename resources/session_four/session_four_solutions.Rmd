...
With `extract()` and `separate()` only one column can be choses, but there are many choices regarding how to split this column into many. With `unite()` however, there are many choices of which columns to combine, but only a single choice as to how to combine their contents.
...
```{r}
heathrow <- read_csv(paste0("https://raw.githubusercontent.com/THargreaves/",
                            "edexcel-large-dataset-extractor/master/output/",
                            "heathrow_2015.csv"), col_types = cols())
leuchars <- read_csv(paste0("https://raw.githubusercontent.com/THargreaves/",
                            "edexcel-large-dataset-extractor/master/output/",
                            "leuchars_2015.csv"), col_types = cols())
```

We'll only keep columns of interest.
...
And we join. Any join will do since the data is complete.

```{r}
weather <- heathrow %>%
  inner_join(leuchars, by = "date", suffix = c('.heathrow', '.leuchars'))
```

We manually specify the suffixes for duplicate column names using `suffix = ...`.

##### Part 3

I will ask how well the temperatures correlate.

```{r}
ggplot(weather, aes(x = mean_temperature.heathrow, 
                    y = mean_temperature.leuchars)) +
  geom_point(col = 'orange') +
  geom_smooth(method = 'lm', col = 'grey30', se = FALSE) + 
  theme_minimal() +
  labs(x = "Mean Temperature in Heathrow (°C)",
       y = "Mean Temperature in Leuchars (°C)",
       title = "Correlation between temperatures in Heathrow and Leuchars")
```
